...
function mapReply(row: Record<string, unknown>): SaveTheDateReply {
  return { id: String(row.id), fullName: String(row.full_name), email: String(row.email), status: row.status as SaveTheDateReply["status"], guestCount: Number(row.guest_count), note: String(row.note ?? ""), createdAt: String(row.created_at), updatedAt: String(row.updated_at) };
}
...
export async function saveReply(input: SaveTheDateInput): Promise<SaveTheDateReply> {
  ...
  const { data, error } = await db
    ...
    .upsert({ full_name: input.fullName, email: input.email, status: input.status, guest_count: input.guestCount, note: input.note, updated_at: new Date().toISOString() }, { onConflict: "email" })
    .select()
    .single();
