...
export function FloralForm() {
  ...
  if (form.saved) {
    return <div className="flo-thanks" role="status">
      ...
      <p className="flo-italic">{form.saved.status === "celebrating" ? `${form.saved.guestCount === 1 ? "One seat is" : `${form.saved.guestCount} seats are`} noted for the celebration — we are so glad.` : "We will miss you on the day, and we are grateful you told us."}</p>
  ...
  return <form className="flo-form" onSubmit={form.submit}>
    ...
    <div className="flo-fields">
      <div className="flo-field"><label className="flo-caps" htmlFor="flo-name">Your name(s)</label><input className="flo-input" id="flo-name" value={form.fullName} onChange={(e) => form.setFullName(e.target.value)} placeholder="The Bennett family" autoComplete="name" required maxLength={120}/></div>
